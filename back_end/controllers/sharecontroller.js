...
const createorder = async (req, res) => {
  ...
  // Validate share size
  if (size <= 0) {
    return res.status(400).json({ message: 'Size cannot be negative or equal to zero.' });
  }
  ...
  try {
    ...
    if (data && data.c && data.c !== 0) {
      ...
      // Update user balance and share ownership
      ...
      return res.status(201).json({ message: 'Order created successfully.', savedShare });
    } else {
      return res.status(400).json({ message: 'Invalid stock symbol. Please check the symbol and try again.' });
    }
  } catch (error) {
